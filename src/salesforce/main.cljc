(ns salesforce.main
  "Kotodama WASM entrypoint for the Salesforce clean-room actor (L4) — Clojure port.
  ...
  facts (`salesforce.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "salesforce")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"      :plural "accounts"      :id-prefix "salesfor_acc"
    :fields [:name :industry :annualRevenue :ownerId :website] :required [:name :industry]
    :coerce {:annualRevenue :float}      :refs {}}
   {:entity "Contact"      :plural "contacts"      :id-prefix "salesfor_con"
    :fields [:accountId :firstName :lastName :email :title]    :required [:firstName :lastName]
    :coerce {}                            :refs {:accountId "Account"}}
   {:entity "Lead"         :plural "leads"         :id-prefix "salesfor_lea"
    :fields [:company :firstName :lastName :status :leadSource] :required [:company :firstName]
    :coerce {}                            :refs {}}
   {:entity "Opportunity"  :plural "opportunities" :id-prefix "salesfor_opp"
    :fields [:accountId :name :stageName :amount :closeDate]    :required [:name :stageName]
    :coerce {:amount :float}              :refs {:accountId "Account"}}
   {:entity "Case"         :plural "cases"         :id-prefix "salesfor_cas"
    :fields [:accountId :subject :status :priority :origin]     :required [:subject :status]
    :coerce {}                            :refs {:accountId "Account"}}
   {:entity "Campaign"     :plural "campaigns"     :id-prefix "salesfor_cam"
    :fields [:name :type :status :budgetedCost]                 :required [:name :type]
    :coerce {:budgetedCost :float}        :refs {}}
   {:entity "Task"         :plural "tasks"         :id-prefix "salesfor_tas"
    :fields [:whoId :subject :status :activityDate]             :required [:subject :status]
    :coerce {}                            :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"salesforce.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "salesforce-compat" :tier tier :entities entities} 200])
...
(defn start! [] :salesforce-compat/ready)
